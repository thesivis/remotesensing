...
#include "./opencl/Constantes.h"
 ...
 void SEBS_Daily_Evapotranspiration_G(
				float day_angle,
				float lat_rad,
				float albedo,
				float SWd24_Wm,
				...
				float evapfr,
				float Ns,
				__global float * Rndaily,
				__global float * Edaily,
				int idx)
{  
	/* const float DT = 60.0*60.0*24.0; */ /* seconds per day */
	
	/* Solar irradiance parameters (FAO 1998, p46/p48) (eq24, eq25 eq31, eq34) */
	float solar_decl = 0.4093f*sin(day_angle-1.405f); /*solar declination P4.31 */
...
*/	
	...
	float enet = -0.02f+0.261f* exp(-7.77e-4f*Ta_av_C*Ta_av_C); /* Net (daily) emissivity */
	...
	Rndaily[idx] = Rn24_Sse + Rn24_L;  /* Net Radiation (MJ m-2 day-1) */
	
	/* Daily Evapotranspiration */
	Edaily[idx] = evapfr * max(Rndaily[idx],0.0f) / ((2.501f-0.002361f*(Ta_av_K-273.15f))*rho_w);  /* Daily Evapotranspiration [mm day-1] */
}

__kernel void SEBS_Daily_Evapotranspiration_Kernel(
				__global int * ComptMask,
				__global float * day_angle,
				__global float * lat_rad,
				__global float * albedo,
				__global float * SWd24_Wm,
				__global float * Ta_av_K,
				__global float * evapfr,
				__global float * Ns,
				__global float * Rndaily,
				__global float * Edaily,
