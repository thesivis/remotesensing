...
*/
	...
	if (zeta > 0.0f)
	{
		...
		return(-(zeta+0.667f*(zeta-5.0f/0.35f)*exp(-0.35f*zeta)+0.667f*5.0f/0.35f)); 
	}
	else
	{
/*		% UNSTABLE conditions(% According to Brutsaert 2008, p50)
		...
		float y_u= min(Y , 14.509365795621073f);
		
		float x_u = cbrt(y_u/0.33f); 

		return(log(0.33f+y_u) -1.2299999892711639f*cbrt(y_u) 
		      +0.1416636768559522f*log((1.0f+x_u)*(1.0f+x_u)/(1.0f-x_u+x_u*x_u)) 
		      +0.4907373718030569f*atan((2.0f*x_u-1.0f)*0.5773502691896258f) 
		      +PSI0);
	}
}
...
/*	% Integrated stability function 1b.
	...
	{
		/* STABLE conditions (According to Beljaars & Holtslag, 1991 eq. 13 ) */
		return( -( pow(1.0f + 2.0f/3.0f * zeta,1.5f) 
		       +0.667f * (zeta - 5.0f / 0.35f)*exp(-0.35f * zeta)
		       +0.667f * 5.0f / 0.35f - 1.0f) );
	}
	else
	{
		/* UNSTABLE conditions (According to Brutsaert 2008, p50) */
		return( ((1.0f - 0.057f) / 0.78f) * log((0.33f + pow(-zeta,0.78f)) / 0.33f) ); 
	}
...
/*	% NOTE: rhoa_WL is only used for the wet-limit. To get a true upperlimit for the sensible heat
	...
	float esat_WL = 611.0f * exp(17.502f * LST_C/(LST_C + 240.97f)); /* saturated vapor pressure [Pa] */
...
/*	% NOTE: esat_WL is only used for the wet-limit. To get a true upperlimit for the sensible heat
	...
	G0[idx] = Rn[idx]*0.34f*exp(-0.46f * LAI); 
	...
	/* ASL height */
	/* hst= alfa*hpbl, with alfa=0.10~0.15 over moderately rough surfaces, or hst=beta*z0, with beta= 100~150. */
	float hst = max(0.12f * hpbl, 125.0f * z0m);
	...
	ustar[idx] = ku / log_z_d0_z0m; /* U* in neutral condition when stability factors are zero */
    float H = CH * ustar[idx] / log_z_d0_z0h; /* H in neutral condition when stability factors are zero */
...
/*	while ( (errorH>0.01) && (steps<1) ) */
	{
		/* Stability Function */
		L = CL * (ustar[idx] * ustar[idx] * ustar[idx])/ H; /* Obukhov Stability length */
		...
		ustar[idx] = ku/(log_z_d0_z0m - bw);
		
		/* update z0h and z0m depending on land cover */
		/* tall vegeation */
		if (Itall)
		{
			z0h = z0m / exp(52.0f*sqrt(0.027f* ustar[idx])/LAI - 0.69f);
		...
		if (I_water)
		{
			z0m = 0.11f*(Nu / ustar[idx] ) + 0.62f*( (ustar[idx] * ustar[idx]) /g); 
			z0h = 0.40f*(Nu / ustar[idx]);
		...
		H = CH * ustar[idx] / (log_z_d0_z0h - cw); 
	...
	{
		re_i[idx]=(log_z_d0_z0h-C_i1+C_i2)/(k * ustar[idx]); /* Actual resistance to heat transfer [s/m] */
	}
	else
	{
		re_i[idx]=(log_z_d0_z0h)/(k * ustar[idx]); /* Actual resistance to heat transfer [s/m] */
	}

	H_i[idx] = rhoa_m_Cp* (Theta_s-Theta_a)/ re_i[idx]; /* Sensible heat flux */
	
	/* Sensible heat flux at theoretical Dry limit */
	/* Dry limit */
	H_DL[idx] = Rn[idx] - G0[idx]; /* Sensible heat at Dry Limit */
	
	/* Sensible heat flux at theoretical wet limit */
	/* Dry air is assumed.. eact=0, and we need to take the density of dry air */
	float L_WL = -(ustar[idx] * ustar[idx] * ustar[idx])* rhoa_WL/(k*g*(0.61f* (Rn[idx] - G0[idx])/L_e)); /* Obukhov stability length at Wet Limit */
	...
	{
		re_WL = (log_z_d0_z0h-C_WL)/(k * ustar[idx]); /* Actual resistance to heat transfer at wet limit(s/m) */
	}
	else
	{
		re_WL = (log_z_d0_z0h)/(k * ustar[idx]); /* Actual resistance to heat transfer at wet limit(s/m) */
	}
	
	H_WL[idx] = ((Rn[idx] - G0[idx]) - (rhoa_WL_Cp/re_WL)*((esat_WL)/ gammaConst))/(1.0f + slope_WL/gammaConst); /* Sensible heat at Wet Limit [W/m2](Su 2002, p88, eq 16) */
	
	/* Evaporative fraction */
	H_i[idx] = min(H_i[idx], H_DL[idx]); /* set lower limit for sensible heat [W/m2] */
	H_i[idx] = max(H_i[idx], H_WL[idx]); /* %set upper limit for sensible heat [W/m2] */
	
	/* Relative evaporation */
	float evap_re = 0.0f;
	if (H_DL[idx] <= H_WL[idx])
	{
		evap_re = 1.0f; /* relative evaporation for water & wet surfaces [] */
	}
	else
	{
		evap_re = 1.0f-(H_i[idx]-H_WL[idx])/(H_DL[idx]-H_WL[idx]); /* relative evaporation for land surface [] */
	}
	...
	{
		evap_fr[idx] = evap_re*(Rn[idx]-G0[idx]-H_WL[idx])/(Rn[idx]-G0[idx]); /* evaporative fraction [] */
	}
	else
	{
	...
	evap_fr[idx] = max(evap_fr[idx],0.0f);
	evap_fr[idx] = min(evap_fr[idx],1.0f);
	
	/* Latent heat flux [W/m2] */
	LE[idx] = evap_fr[idx] * (Rn[idx] - G0[idx]); 
	
}


__kernel void SEBS_EnergyBalance_Kernel(
			...
			__global int * DataSize)
{
	...
	__local float sLAI;
	__local float shc;
	__local float salbedo;
	__local float semissivity;
